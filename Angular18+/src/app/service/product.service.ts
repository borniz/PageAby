...
export class ProductService {
  ...
  getByIdProduct(id: string): Observable<IProduct> {
    return this.http.get<IProduct>(`${this.apiUrl}/${id}`, { headers: this.headers });
  }

  createProduct(product: Omit<IProduct, 'id' | 'img'>): Observable<IProduct> {
    return this.http.post<IProduct>(this.apiUrl, product, { headers: this.headers });
  }

  updateProduct(id: string, product: Partial<IProduct>): Observable<IProduct> {
    return this.http.put<IProduct>(`${this.apiUrl}/${id}`, product, { headers: this.headers });
  }
  ...
  uploadImage(file: File, productId: string): Observable<any> {
    ...
    return this.http.post(`${this.apiUrl}/${productId}/upload-image`, formData, { headers: this.headers });
  }
